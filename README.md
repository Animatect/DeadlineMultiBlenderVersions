...
The modification made to the submission process is basically that when the blender client addon calls the submitBlenderToDeadline Script, we capture the blender version, pass it piggybagged in the output file argument to the BlenderSubmission script and there we write it to the .job file to pass it to the Blender.py script where we modify the logic to take the version into account.
